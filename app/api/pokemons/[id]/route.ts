...
export async function GET(
  ...
  if (abilityLinks && abilityLinks.length > 0) {
    const abilityIds = (abilityLinks as AbilityLinkRow[]).map((a) => a.abilityId);
    ...
    if (abilErr) {
      ...
      return NextResponse.json<ApiErrorResponse>(
        { error: "특성 상세 정보 조회 중 오류가 발생했습니다." },
        { status: 500 }
      );
    }

    // abilityId → 상세 정보 맵
    const abilityDetailMap = new Map<number, AbilityRow>(
      ((abilityDetails ?? []) as AbilityRow[]).map((a) => [a.id, a])
    );

    // 특성 링크 순서 유지하며 조립 (isHidden=false 먼저)
    abilities = (abilityLinks as AbilityLinkRow[])
      .map((link): PokemonAbilityInfo | null => {
        const detail = abilityDetailMap.get(link.abilityId);
        if (!detail) return null;

        const displayName =
          detail.koreanName ?? detail.altKorName ?? `ability_${detail.id}`;
        const displayDescription =
          detail.korDescription ?? detail.altKorDescription ?? null;
