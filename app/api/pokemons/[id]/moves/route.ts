...
export async function GET(request: NextRequest, {params}: {params: Promise<{id: string}>}) {
  ...
  if (learnErr) {
    ...
    return NextResponse.json<ApiErrorResponse>({error: "기술 학습 데이터 조회 중 오류가 발생했습니다."}, {status: 500});
  }
  ...
  if (moveErr) {
    ...
    return NextResponse.json<ApiErrorResponse>({error: "기술 상세 정보 조회 중 오류가 발생했습니다."}, {status: 500});
  }

  // Step 5: typeId → 한국어 타입명 맵 조회
  const typeIds = [
    ...new Set(((moveDetails ?? []) as MoveDetailRow[]).map((m) => m.typeId).filter((id): id is number => id != null)),
  ];
  const typeMap = await fetchTypeMap(typeIds);

  // Step 6: 응답 조립
  const moveDetailMap = new Map<number, MoveDetailRow>(((moveDetails ?? []) as MoveDetailRow[]).map((m) => [m.id, m]));
